'use client';
import BASE_URL from '@/app/lib/config';
import React, { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';

const RequestPasien = () => {
  const [requests, setRequests] = useState([]);
  const [searchDate, setSearchDate] = useState('');
  const router = useRouter();

  
  useEffect(() => {
    const userId = localStorage.getItem('userId');
    if (!userId) {
      router.push('/');  
      return;
    }

    const fetchRequests = async () => {
      try {
        const response = await fetch(`${BASE_URL}/request_pasien_data.php?patient_id=${userId}`);
        const data = await response.json();
        if (data.status === 'success') {
          console.log("Fetched requests:", data.requests); 
          setRequests(data.requests);
        } else {
          console.error('Server returned error:', data.message);
          alert('Failed to fetch data: ' + data.message);
        }
      } catch (error) {
        console.error("Error fetching data:", error);
        alert('Network error: Unable to connect to server');
      }
    };

    fetchRequests();
    const interval = setInterval(() => {
      fetchRequests();
    }, 5000); 
  
    return () => clearInterval(interval);
  }, [router]);

  
  const handleAction = async (requestId, action) => {
    try {
      const response = await fetch(`${BASE_URL}/request_pasien_action.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requestId, action })
      });

      const data = await response.json();
      console.log('Action response:', data); 

      if (data.status === 'success') {
        
        setRequests(prevRequests => 
          prevRequests.map(req => {
            if (req.izin_pasien_id === requestId) {
              return {
                ...req,
                status: action === 'accept' ? 1 : 0
              };
            }
            return req;
          })
        );
        alert(data.message);
      } else {
        alert('Failed to update request: ' + data.message);
      }
    } catch (error) {
      console.error('Error handling action:', error);
      alert('Error processing action: ' + error.message);
    }
  };

  
  const filteredRequests = requests.filter(entry =>
    entry.dokter_nama.toLowerCase().includes(searchDate.toLowerCase())
  );

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap');

        :root {
          --blue-light: #60a5fa;
          --blue-lighter: #bfdbfe;
          --gray-dark: #374151;
          --gray-medium: #6b7280;
          --input-bg: #f9fafb;
          --border-radius: 0.75rem;
          --shadow-light: 0 4px 6px rgba(96, 165, 250, 0.1);
          --transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }

        body, html {
          margin: 0;
          font-family: 'Poppins', sans-serif;
          background-color: #f9fafb;
          color: var(--gray-dark);
        }

        .container {
          display: flex;
          height: 100vh;
        }

        .sidebar {
          width: 250px;
          background-color: var(--blue-light);
          color: white;
          display: flex;
          flex-direction: column;
          justify-content: space-between;
          padding: 2rem;
          box-shadow: 2px 0 8px rgba(0, 0, 0, 0.1);
        }

        .sidebar h2 {
          font-size: 1.5rem;
          margin-bottom: 2rem;
        }

        .menu {
          display: flex;
          flex-direction: column;
        }

        .menu-item {
          font-size: 1.1rem;
          margin: 1rem 0;
          cursor: pointer;
          transition: all var(--transition);
          padding: 0.5rem 1rem;
          border-radius: var(--border-radius);
        }

        .menu-item:hover {
          background-color: rgba(255, 255, 255, 0.1);
          transform: translateX(5px);
        }

        .main-content {
          flex-grow: 1;
          padding: 2rem;
        }

        .search-bar {
          margin-bottom: 2rem;
          display: flex;
          justify-content: space-between;
          align-items: center;
        }

        .search-input {
          padding: 0.75rem 1rem;
          width: 200px;
          font-size: 1rem;
          border-radius: var(--border-radius);
          border: 1.5px solid var(--blue-lighter);
          background-color: var(--input-bg);
          color: var(--gray-dark);
          transition: all var(--transition);
        }

        .search-input:focus {
          border-color: var(--blue-light);
          box-shadow: 0 0 0 3px rgba(96, 165, 250, 0.2);
          outline: none;
        }

        .history-table-container {
          max-height: 60vh;
          overflow-y: auto;
          margin-top: 2rem;
          border-radius: var(--border-radius);
          box-shadow: var(--shadow-light);
        }

        .history-table {
          width: 100%;
          border-collapse: collapse;
          background: white;
        }

        .history-table th {
          position: sticky;
          top: 0;
          background-color: var(--blue-light);
          color: white;
          font-weight: 600;
          z-index: 1;
          padding: 1rem;
          text-align: center;
        }

        .history-table td {
          padding: 1rem;
          text-align: center;
          border-bottom: 1px solid #e5e7eb;
          transition: all var(--transition);
        }

        .history-table tr:hover td {
          background-color: #f3f4f6;
        }

        .history-title {
          font-size: 2rem;
          font-weight: 600;
          color: var(--blue-light);
          margin-bottom: 1rem;
        }

        .action-button {
          padding: 0.5rem 1rem;
          border: none;
          border-radius: var(--border-radius);
          cursor: pointer;
          font-weight: 600;
          transition: all var(--transition);
          margin-right: 0.5rem;
        }

        .accept-button {
          background-color: #10b981;
          color: white;
        }

        .accept-button:hover {
          background-color: #059669;
          transform: translateY(-2px);
          box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
        }

        .reject-button {
          background-color: #ef4444;
          color: white;
        }

        .reject-button:hover {
          background-color: #dc2626;
          transform: translateY(-2px);
          box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
        }

        .status-badge {
          padding: 0.25rem 0.75rem;
          border-radius: 9999px;
          font-size: 0.875rem;
          font-weight: 500;
          display: inline-block;
        }

        .status-pending {
          background-color: #fef3c7;
          color: #f59e0b;
        }

        .status-accepted {
          background-color: #d1fae5;
          color: #10b981;
        }

        .status-rejected {
          background-color: #fee2e2;
          color: #ef4444;
        }
      `}</style>

      <div className="container">
        <div className="sidebar">
          <h2>Patient Dashboard</h2>
          <div className="menu">
            <div className="menu-item" onClick={() => router.push('/homePasien')}>Home</div>
            <div className="menu-item" onClick={() => router.push('/historyPasien')}>History</div>
            <div className="menu-item" onClick={() => router.push('/requestPasien')}>Permission</div>
          </div>
        </div>

        <div className="main-content">
          <h1 className="history-title">Request Patients Permission</h1>

          <div className="search-bar">
            <input
              type="text"
              className="search-input"
              placeholder="Search by Doctor's Name"
              value={searchDate}
              onChange={(e) => setSearchDate(e.target.value)}
            />
          </div>

          <div className="history-table-container">
            <table className="history-table">
              <thead>
                <tr>
                  <th>No.</th>
                  <th>Status</th>
                  <th>Doctor</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredRequests.map((entry, index) => (
                  <tr key={entry.izin_pasien_id}>
                    <td>{index + 1}</td>
                    <td>
                      {entry.status == 0 ? (
                        <span className="status-badge status-rejected">Rejected</span>
                      ) : entry.status == 1 ? (
                        <span className="status-badge status-accepted">Accepted</span>
                      ) : entry.status == 2 ? (
                        <span className="status-badge status-pending">Pending</span>
                      ) : (
                        <span className="status-badge">Unknown</span>
                      )}
                    </td>
                    <td>dr. {entry.dokter_nama}</td>
                    <td>
                      {parseInt(entry.status) === 2 && (
                        <>
                          <button 
                            className="action-button accept-button"
                            onClick={() => handleAction(entry.izin_pasien_id, 'accept')}
                          >
                            Accept
                          </button>
                          <button 
                            className="action-button reject-button"
                            onClick={() => handleAction(entry.izin_pasien_id, 'reject')}
                          >
                            Reject
                          </button>
                        </>
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </>
  );
};

export default RequestPasien;

